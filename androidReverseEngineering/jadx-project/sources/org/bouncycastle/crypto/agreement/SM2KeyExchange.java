...
public class SM2KeyExchange {
    ...
    private BigInteger reduce(BigInteger bigInteger) {
        return bigInteger.and(BigInteger.valueOf(1L).shiftLeft(this.f1123w).subtract(BigInteger.valueOf(1L))).setBit(this.f1123w);
    }
    ...
    public byte[] calculateKey(int i2, CipherParameters cipherParameters) {
        ...
        byte[] z3 = getZ(this.digest, bArr, sM2KeyExchangePublicParameters.getStaticPublicKey().getQ());
        ECPoint calculateU = calculateU(sM2KeyExchangePublicParameters);
        return this.initiator ? kdf(calculateU, z2, z3, i2) : kdf(calculateU, z3, z2, i2);
    }
    ...
    public byte[][] calculateKeyWithConfirmation(int i2, byte[] bArr, CipherParameters cipherParameters) {
        ...
        byte[] z2 = getZ(this.digest, this.userID, this.staticPubPoint);
        byte[] z3 = getZ(this.digest, bArr2, sM2KeyExchangePublicParameters.getStaticPublicKey().getQ());
        ECPoint calculateU = calculateU(sM2KeyExchangePublicParameters);
        if (!this.initiator) {
            byte[] kdf = kdf(calculateU, z3, z2, i2);
            byte[] calculateInnerHash = calculateInnerHash(this.digest, calculateU, z3, z2, sM2KeyExchangePublicParameters.getEphemeralPublicKey().getQ(), this.ephemeralPubPoint);
            return new byte[][]{kdf, S1(this.digest, calculateU, calculateInnerHash), S2(this.digest, calculateU, calculateInnerHash)};
        }
        byte[] kdf2 = kdf(calculateU, z2, z3, i2);
        byte[] calculateInnerHash2 = calculateInnerHash(this.digest, calculateU, z2, z3, this.ephemeralPubPoint, sM2KeyExchangePublicParameters.getEphemeralPublicKey().getQ());
    ...
    public void init(CipherParameters cipherParameters) {
        ...
        this.f1123w = (this.ecParams.getCurve().getFieldSize() / 2) - 1;
    }
