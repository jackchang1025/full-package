...
public class DSASigner implements DSAExt {
    ...
    public BigInteger[] generateSignature(byte[] bArr) {
        ...
        if (this.kCalculator.isDeterministic()) {
            this.kCalculator.init(q, x2, bArr);
        } else {
        ...
        return new BigInteger[]{mod, BigIntegers.modOddInverse(q, nextK).multiply(calculateE.add(x2.multiply(mod))).mod(q)};
    }
    ...
    public boolean verifySignature(byte[] bArr, BigInteger bigInteger, BigInteger bigInteger2) {
        ...
        return parameters.getG().modPow(mod, p2).multiply(((DSAPublicKeyParameters) this.key).getY().modPow(mod2, p2)).mod(p2).mod(q).equals(bigInteger);
    }
