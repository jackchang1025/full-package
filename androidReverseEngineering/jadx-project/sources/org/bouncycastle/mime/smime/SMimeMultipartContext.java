...
public class SMimeMultipartContext implements MimeMultipartContext {
    ...
    private DigestCalculator[] createDigestCalculators(Headers headers) {
        try {
            ...
            for (int i2 = 0; i2 < split.length; i2++) {
                digestCalculatorArr[i2] = this.parserContext.getDigestCalculatorProvider().get(new AlgorithmIdentifier(SMimeUtils.getDigestOID(SMimeUtils.lessQuotes(split[i2]).trim())));
            }
    ...
    public OutputStream getDigestOutputStream() {
        ...
        while (i2 < this.calculators.length) {
            TeeOutputStream teeOutputStream = new TeeOutputStream(this.calculators[i2].getOutputStream(), outputStream);
            i2++;
