...
class Utils {
        ...
        return (List) AccessController.doPrivileged(new PrivilegedAction<List<T>>() { // from class: org.bouncycastle.oer.its.Utils.1
            ...
            public List<T> run() {
                try {
                    ...
                    throw new IllegalStateException(AbstractC0000a.m9e(e2, new StringBuilder("could not invoke getInstance on type ")), e2);
                }
            }
        });
    }
    ...
    public static byte[] octetStringFixed(byte[] bArr, int i2) {
        if (bArr.length == i2) {
            return bArr;
        }
        throw new IllegalArgumentException("octet string out of range");
