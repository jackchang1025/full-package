...
public class PKIXNameConstraintValidator implements NameConstraintValidator {
    ...
    private byte[][] minMaxIPs(byte[] bArr, byte[] bArr2, byte[] bArr3, byte[] bArr4) {
        ...
        for (int i2 = 0; i2 < length; i2++) {
            ...
            byte b = bArr[i2];
            byte b2 = bArr2[i2];
            bArr6[i2] = (byte) ((b & b2) | (~b2));
            bArr7[i2] = (byte) (bArr3[i2] & bArr4[i2]);
            byte b3 = bArr3[i2];
            byte b4 = bArr4[i2];
            bArr8[i2] = (byte) ((b3 & b4) | (~b4));
        }
    ...
    private String stringifyIPCollection(Set set) {
        StringBuilder m20p = AbstractC0000a.m20p("[");
        Iterator it = set.iterator();
        while (it.hasNext()) {
            if (m20p.length() > 1) {
                m20p.append(",");
            }
            m20p.append(stringifyIP((byte[]) it.next()));
        }
        m20p.append("]");
    ...
    private String stringifyOtherNameCollection(Set set) {
        ...
        while (it.hasNext()) {
            ...
            m20p.append(otherName.getTypeID().getId());
            m20p.append(":");
            try {
                m20p.append(Hex.toHexString(otherName.getValue().toASN1Primitive().getEncoded()));
            } catch (IOException e2) {
                m20p.append(e2.toString());
            }
        }
        m20p.append("]");
        return m20p.toString();
    ...
    private boolean withinDomain(String str, String str2) {
        ...
        for (int i2 = -1; i2 < split.length; i2++) {
            if (i2 == -1) {
                if (split2[i2 + length].equals(BuildConfig.FLAVOR)) {
                    return false;
                }
    ...
    public void intersectEmptyPermittedSubtree(int i2) {
        ...
        if (i2 == 4) {
            ...
            if (i2 != 7) {
                throw new IllegalStateException(AbstractC0000a.m11g("Unknown tag encountered: ", i2));
            }
    ...
    public void intersectPermittedSubtree(GeneralSubtree[] generalSubtreeArr) {
        ...
        for (Map.Entry entry : hashMap.entrySet()) {
            ...
            if (intValue == 0) {
                ...
                if (intValue != 7) {
                    throw new IllegalStateException(AbstractC0000a.m11g("Unknown tag encountered: ", intValue));
                }
