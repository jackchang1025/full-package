...
/**
 * WIFIBackgroundService — JobService that acquires a wifi lock to keep wifi alive.
 * Used to maintain network connectivity in the background.
 */
public class WIFIBackgroundService extends JobService {
    private static final String TAG = "WIFIBackgroundService";
    ...
    public final void onCreate() {
        ...
        Log.i(TAG, var1.toString());
        this.a = ((WifiManager) this.getApplicationContext().getSystemService("wifi"))
                .createWifiLock(3, "MyWifiLockTag");
    }
    ...
    public final int onStartCommand(Intent var1, int var2, int var3) {
        Log.i(TAG, "onStartCommand - startId = " + var3 + ", Thread ID = " + Thread.currentThread().getId());
        try {
            ...
            Log.w(TAG, "WAKE_LOCK/WIFI_STATE permission not granted", ex);
        }
        return 1;
    }
