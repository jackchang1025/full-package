...
class MiuiStepsTest {
    ...
    fun `AUTOSTART_COMPONENTS has correct package`() {
        assertTrue(MiuiSteps.AUTOSTART_COMPONENTS.all { c -> c.packageName == "com.miui.securitycenter" })
    }

    @Test
    fun `AUTOSTART_COMPONENTS has correct class names`() {
        val classNames = MiuiSteps.AUTOSTART_COMPONENTS.map { c -> c.className }
        assertTrue(classNames.any { name -> name.contains("AutoStartManagementActivity") })
    }

    @Test
    fun `AUTOSTART_COMPONENTS has 2 entries`() {
        assertEquals(2, MiuiSteps.AUTOSTART_COMPONENTS.size)
    }
    ...
    fun `BATTERY_COMPONENTS has correct entries`() {
        assertEquals(2, MiuiSteps.BATTERY_COMPONENTS.size)
        assertTrue(MiuiSteps.BATTERY_COMPONENTS.any { c -> c.packageName == "com.miui.powerkeeper" })
        assertTrue(MiuiSteps.BATTERY_COMPONENTS.any { c -> c.packageName == "com.miui.securitycenter" })
    }

    @Test
    fun `BATTERY_COMPONENTS contains HiddenAppsConfigActivity`() {
        assertTrue(MiuiSteps.BATTERY_COMPONENTS.any { c -> c.className.contains("HiddenAppsConfigActivity") })
    }
    ...
    fun `AUTOSTART_KEYWORDS contains Chinese keywords`() {
        assertTrue(MiuiSteps.AUTOSTART_KEYWORDS.contains("自启动"))
        assertTrue(MiuiSteps.AUTOSTART_KEYWORDS.contains("自动启动"))
    }

    @Test
    fun `AUTOSTART_KEYWORDS contains English keywords`() {
        assertTrue(MiuiSteps.AUTOSTART_KEYWORDS.contains("Auto-start"))
        assertTrue(MiuiSteps.AUTOSTART_KEYWORDS.contains("Autostart"))
        assertTrue(MiuiSteps.AUTOSTART_KEYWORDS.contains("Auto start"))
    }

    @Test
    fun `BATTERY_NO_RESTRICT_KEYWORDS contains expected keywords`() {
        assertTrue(MiuiSteps.BATTERY_NO_RESTRICT_KEYWORDS.contains("无限制"))
        assertTrue(MiuiSteps.BATTERY_NO_RESTRICT_KEYWORDS.contains("No restrictions"))
        assertTrue(MiuiSteps.BATTERY_NO_RESTRICT_KEYWORDS.contains("Unrestricted"))
        assertTrue(MiuiSteps.BATTERY_NO_RESTRICT_KEYWORDS.contains("不限制"))
    }

    @Test
    fun `BG_POPUP_KEYWORDS contains expected keywords`() {
        assertTrue(MiuiSteps.BG_POPUP_KEYWORDS.contains("后台弹出界面"))
        assertTrue(MiuiSteps.BG_POPUP_KEYWORDS.contains("后台弹窗"))
        assertTrue(MiuiSteps.BG_POPUP_KEYWORDS.contains("Background pop-up"))
        assertTrue(MiuiSteps.BG_POPUP_KEYWORDS.contains("Display pop-up"))
    }
    ...
    fun `launchComponentActivity returns true on first success`() {
        // Robolectric context doesn't throw for unknown components
        val components = listOf(
            ComponentName("com.test", "com.test.Activity1"),
            ComponentName("com.test", "com.test.Activity2")
        )
